...
class SelectTestOptionTransformer(BaseEstimator,TransformerMixin):
    ...
    def __init__(self,exclude=[]):
        ...
        self.test_option_index = {'AMB_TEMP': 0, 'CH4': 1, 'CO': 2, 'NMHC': 3, 'NO': 4, 
                              'NO2': 5, 'NOx': 6, 'O3': 7, 'PM10': 8, 'PM2.5': 9, 
                              'RAINFALL': 10, 'RH': 11, 'SO2': 12, 'THC': 13, 
                              'WD_HR': 14, 'WIND_DIREC': 15, 'WIND_SPEED': 16, 
                              'WS_HR': 17}
...
def plot_validation_curve(train_scores,valid_scores,param_range,ylim=None):
...
class SearchUtils() :
    ...
    ## 定义搜索函数 ##
    def search(self,model,param,param_range,ylim=(0.7,1),xlabel=None,cv=None,X=None):
        if len(X) == 0 :
            X = self.X
        if model == None :
           model = self.model 
